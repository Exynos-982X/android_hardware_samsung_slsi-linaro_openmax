...
OMX_ERRORTYPE ExynosOMXUtils::makeComponentInstance(
        const char *name,
...
OMX_ERRORTYPE ExynosOMXUtils::destroyComponentInstance(
        OMX_COMPONENTTYPE *component) {
    if (mLibHandle == NULL) {
        return OMX_ErrorUndefined;
    }
...
OMX_ERRORTYPE ExynosOMXUtils::enumerateComponents(
        OMX_STRING name,
        size_t size,
        OMX_U32 index) {
    if (mLibHandle == NULL) {
...
OMX_ERRORTYPE ExynosOMXUtils::getRolesOfComponent(
        const char *name,
